...
from scripts.add_resource import append_to_csv  # noqa
...
def test_append_to_csv_date_fields(temp_csv: Path, sample_resource_data: dict[str, str]) -> None:
...
def test_append_to_csv_handles_csv_error(sample_resource_data: dict[str, str]) -> None:
    """Test that append_to_csv handles file write errors gracefully."""
    with patch("scripts.add_resource.os.path.join") as mock_join:
        # Point to a non-writable location
        mock_join.return_value = "/invalid/path/to/csv.csv"
